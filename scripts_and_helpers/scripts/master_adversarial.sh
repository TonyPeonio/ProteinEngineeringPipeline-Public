...
for (( GEN=$START_GEN; GEN<=NUM_GENERATIONS; GEN++ )); do
    ...
    conda activate "$COLAB_DIR"
    # Execute ColabFold on the 10 mutant FASTAs. 
    # Adjust the fasta/output paths if your folder names differ slightly!
    colabfold_batch "$PROJECT_ROOT/phase2_fastas" "$OUTPUT_DIR/colabfold_phase2_results" \
    ...
    # Copy the winning drug and the new mutant target into the safe archive
    cp "$PROJECT_ROOT/evaluate_drug_results/lead_drug.pdb" "$GEN_DIR/lead_drug_gen${GEN}.pdb"
    cp "$PROJECT_ROOT/evaluate_cancer_results/next_gen_target.pdb" "$GEN_DIR/mutant_target_gen${GEN}.pdb"

    # Swap the target for the next generation!
    TARGET_PDB="$PROJECT_ROOT/evaluate_cancer_results/next_gen_target.pdb"
    
    # --- PHASE 4: CLEANUP ---
    # Wipe the output folders so the next generation doesn't re-process old files!
    ...
    rm -rf "$PROJECT_ROOT/phase2_fastas/"*
    rm -rf "$OUTPUT_DIR/rfdiffusion/"*
    # Add any other folders (like MPNN fasta output dirs) that need to be cleared here
